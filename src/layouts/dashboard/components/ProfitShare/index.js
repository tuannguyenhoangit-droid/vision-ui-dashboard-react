...
export function ProfitShare({ open, onClose = () => null, data = null }) {
    const { symbol, unRealizedProfit, entryPrice, markPrice, margin, notional, leverage, positionInitialMargin, positionAmt } = data || {};
    ...
    return (
        <Dialog onClose={onClose} open={open} maxWidth PaperProps={{
            style: {
            ...
            <GradientBorder borderRadius={20} fullWidth="100%">
                ...
                >
                    ...
                    <VuiBox mt={2} display="flex" alignItems="center" gap="16px" flexDirection="row">
                        <VuiTypography variant="h6" color={parseInt(positionAmt) >= 0 ? "success" : "error"} fontWeight="bold">
                            {parseInt(positionAmt) >= 0 ? "LONG" : "SHORT"}
                        </VuiTypography>
                        <VuiTypography color="white" variant="caption" fontWeight="normal">|</VuiTypography>
                        <VuiTypography variant="h6" color="white" fontWeight="bold">
                            {[Math.round(parseInt(notional) / parseFloat(positionInitialMargin)).toFixed(0), 'x'].join(' ')}
                        </VuiTypography>
                        <VuiTypography color="white" variant="caption" fontWeight="normal">|</VuiTypography>
                        <VuiTypography variant="h6" color="white" fontWeight="bold">
                            {symbol}
                    ...
                    <VuiBox mt={1} display="flex" alignItems="center" gap="16px" flexDirection="row">
                        <VuiTypography variant="h3" color={parseFloat(unRealizedProfit) > 0 ? "success" : "error"} fontWeight="bold">
                            {[unRealizedProfit > 0 ? '+' : '', ((parseFloat(unRealizedProfit) / parseFloat(positionInitialMargin)) * 100).toFixed(2), '%'].join(' ')}
                        </VuiTypography>
                    </VuiBox>
                    ...
                    <VuiBox mt={2} gap="16px" flexDirection="column">
                        <VuiBox display="flex">
                            <VuiTypography variant="caption" color="white">
                                Link Binance API Key and enjoy SA BOT at
                            </VuiTypography>
                        </VuiBox>
                        <VuiBox display="flex" mt={0.5}>
